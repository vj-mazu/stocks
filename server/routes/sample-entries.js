      ...
      hydrateOptions: {
        ...
        include: [
          ...
          { model: User, as: 'creator', attributes: ['id', 'username', 'fullName'] },
          {
            model: LotAllotment,
            as: 'lotAllotment',
            attributes: ['id', 'closedReason', 'closedAt', 'closedByUserId', 'completionType', 'allottedBags', 'inspectedBags', 'samplingRulesMode', 'allottedToSupervisorId', 'allottedByManagerId'],
            required: false
          }
        ],
        subQuery: false
    ...
    upload.single('photo')(req, res, async (err) => {
      ...
      try {
        ...
        const qualityData = {
          ...
          moisture: hasMoisture ? parseFloatSafe(req.body.moisture) : (isPrepOr100gSave ? prevQ.moisture : null),
          dryMoisture: hasDryMoisture ? parseFloatSafe(req.body.dryMoisture) : (isPrepOr100gSave ? prevQ.dryMoisture : null),
          cutting1: hasCutting1 ? parseFloatSafe(req.body.cutting1) : (isPrepOr100gSave ? prevQ.cutting1 : null),
          cutting2: hasCutting2 ? parseFloatSafe(req.body.cutting2) : (isPrepOr100gSave ? prevQ.cutting2 : null),
          ...
          cutting2Raw: hasCutting2 ? normalizeRaw(req.body.cutting2) : (isPrepOr100gSave ? prevQ.cutting2Raw : null),
          bend1Raw: hasBend1 ? normalizeRaw(req.body.bend1) : (isPrepOr100gSave ? prevQ.bend1Raw : null),
          bend2Raw: hasBend2 ? normalizeRaw(req.body.bend2) : (isPrepOr100gSave ? prevQ.bend2Raw : null),
          mixSRaw: hasSmix ? (smixEnabled ? normalizeRaw(req.body.mixS) : null) : (isPrepOr100gSave ? prevQ.mixSRaw : null),
          mixLRaw: hasLmix ? (lmixEnabled ? normalizeRaw(req.body.mixL) : null) : (isPrepOr100gSave ? prevQ.mixLRaw : null),
          mixRaw: hasMix ? normalizeRaw(req.body.mix) : (isPrepOr100gSave ? prevQ.mixRaw : null),
          kanduRaw: hasKandu ? normalizeRaw(req.body.kandu) : (isPrepOr100gSave ? prevQ.kanduRaw : null),
          oilRaw: hasOil ? normalizeRaw(req.body.oil) : (isPrepOr100gSave ? prevQ.oilRaw : null),
          skRaw: hasSk ? normalizeRaw(req.body.sk) : (isPrepOr100gSave ? prevQ.skRaw : null),
          grainsCountRaw: hasGrains ? normalizeRaw(req.body.grainsCount) : (isPrepOr100gSave ? prevQ.grainsCountRaw : null),
          wbRRaw: hasWbR ? (wbEnabled ? normalizeRaw(req.body.wbR) : null) : (isPrepOr100gSave ? prevQ.wbRRaw : null),
...
router.post('/:id/resume-lot', authenticateToken, async (req, res) => {
  try {
    ...
    // Clear closed state but KEEP the close reason for history
    await LotAllotmentService.updateLotAllotment(
      existingAllotment.id,
      {
        closedAt: null,
        closedByUserId: null,
        // NOTE: closedReason is NOT cleared - it persists for display in Lot Reason column
...
router.post('/:id/final-rate-2', authenticateToken, async (req, res) => {
  try {
    const sampleEntryId = req.params.id;
    const {
      finalBaseRate2, finalSute2, finalSuteUnit2, finalPrice2,
      hamali2, hamaliUnit2, brokerage2, brokerageUnit2,
      lf2, lfUnit2, egbValue2, egbType2,
      cdValue2, cdUnit2, bankLoanValue2, bankLoanUnit2,
      paymentConditionValue2, paymentConditionUnit2,
      finalRemarks2
    } = req.body;
    ...
    // Find or create offering record
    let offering = await SampleEntryOffering.findOne({ where: { sampleEntryId } });
