...
router.post('/:id/place', auth, requireInventoryRole, async (req, res) => {
  ...
  try {
    ...
    if (inspection) {
      ...
      // No approval workflow - entry stays in In-Transit until manually moved to Band Malal Book
      ...
      return res.json({ message: 'Place added successfully — entry is in In-Transit', detail });
...
router.post('/:id/wb', auth, async (req, res) => {
  ...
  try {
    ...
    const autoApproveRoles = ['admin', 'ceo', 'manager'];
    ...
    const isAutoApproveWb = autoApproveRoles.includes(userRole) || autoApproveRoles.includes(effectiveRole);



    const targetWbStatus = isAutoApproveWb ? 'approved' : 'pending';
    ...
    if (transitDetail) {
      ...
      if (partyWbName) {
        ...
        const isAutoApproveWb = ['admin', 'ceo', 'manager'].includes(String(userRole || '').toLowerCase()) || ['ceo'].includes(String(effectiveRole || '').toLowerCase());
