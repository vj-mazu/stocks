...
router.get('/in-transit', auth, async (req, res) => {
  ...
  try {
    ...
    const approvedArrivals = arrivals.filter(item => item && item.isFullLorryApproved);
    res.json({ arrivals: approvedArrivals, total: approvedArrivals.length });
    ...
    const arrivals = await Promise.all(entries.map(async (detail, index) => {
      ...
      try {
        ...
        return {
          ...
          debugError: entryError.message,



          debugStack: entryError.stack
