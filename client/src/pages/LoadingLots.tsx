...
const LoadingLots: React.FC<LoadingLotsProps> = ({ entryType, excludeEntryType }) => {
  ...
  const handleSaveValues = async () => {
    ...
    try {
      ...
      setSelectedEntry(null);
      fetchEntries();
      showNotification((response.data as any)?.message || 'Values saved successfully', 'success');
    } catch (error: any) {
  ...
  const modalSuteMissing = !!selectedEntry && !parseFloat(modalOffering.finalSute ?? '') && !parseFloat(modalOffering.sute ?? '');
  const modalMoistureMissing = !!selectedEntry && !parseFloat(modalOffering.moistureValue ?? '');
  const modalHamaliMissing = !!selectedEntry && !hasPositiveAmount(modalOffering.hamali ?? modalOffering.hamaliPerKg);
  const modalBrokerageMissing = !!selectedEntry && !parseFloat(modalOffering.brokerage ?? '');
  const modalLfMissing = !!selectedEntry && modalHasLf && !parseFloat(modalOffering.lf ?? '');
                      ...
                      <div key={brokerName} style={{ display: 'inline-block', minWidth: '100%', marginBottom: 0 }}>
                          ...
                          <tbody>
                            {orderedEntries.map((entry, index) => {
                              ...
                              const isApprovedByManager = String(o.pendingManagerValueApprovalStatus || '').toLowerCase() === 'approved';
                              ...
                              if (isRiceMode) {
                                return (
                                  <tr key={entry.id} style={{ background: rowBg }}>
                                    ...
                                    <td style={{ border: '1px solid #000', padding: '3px 5px', textAlign: 'left', fontSize: '11px', lineHeight: '1.3', wordBreak: 'break-word', background: rowBg }}>
                                      {entry.lotAllotment?.closedReason ? (
                                        <span style={{ color: '#b91c1c', fontWeight: 600, fontSize: '11px' }}>{entry.lotAllotment.closedReason}</span>
                                      ) : (
                              ...
                              return (
                                <tr key={entry.id} style={{ background: rowBg }}>
                                  ...
                                  <td style={{ border: '1px solid #000', padding: '3px 5px', textAlign: 'left', fontSize: '11px', lineHeight: '1.3', wordBreak: 'break-word', background: rowBg }}>
                                    {entry.lotAllotment?.closedReason ? (
                                      <span style={{ color: '#b91c1c', fontWeight: 600 }}>{entry.lotAllotment.closedReason}</span>
                                    ) : (
                                      <span style={{ color: '#999', fontSize: '10px' }}>-</span>
                                    )}
                                  </td>
                                  ...
                                  <td style={{ border: '1px solid #000', padding: '4px 6px', textAlign: 'center', background: '#fafcff' }}>
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', alignItems: 'stretch' }}>
                                      ...
                                      <div style={{ fontSize: '10px', fontWeight: 700, color: managerApprovalPending ? '#6d28d9' : (needsFill ? '#856404' : '#155724'), background: managerApprovalPending ? '#ede9fe' : (needsFill ? '#fff3cd' : '#d4edda'), border: managerApprovalPending ? '1px solid #c4b5fd' : (needsFill ? '1px solid #ffeeba' : '1px solid #c3e6cb'), borderRadius: '4px', padding: '2px 4px', lineHeight: '1.25' }}>
                                        {managerApprovalPending ? 'Manager Added Pending Approval' : (needsFill ? `Missing: ${compactStatusText(missingFieldLabels)}` : (isApprovedByManager ? 'Manager Added' : 'Admin Added'))}
                                      </div>
                                    </div>
                                  </td>
                                  <td style={{ border: '1px solid #000', padding: '6px', textAlign: 'center', minWidth: '96px' }}>
